...
import PatientDetails.PatientPDFExtract;
...
//Declare a static flag outside for the stop print the same word in report again and again
	...
	public Object[][] getDataForKNR_RC() throws IOException {
		Map<String, KnrRCReadExcelFiles.StudentInfo> groupedData = KnrRCReadExcelFiles.readExcel("excel.file1",
				"excel.file1");
		...
		for (Map.Entry<String, KnrRCReadExcelFiles.StudentInfo> entry : groupedData.entrySet()) {
			...
			// You can extract or loop through subject keys to generate subject-specific
			// data.
...
//	    TRData.ResultDeliverablesTRDataClgWiseNavigation();
	...
	public void testCase6(String regNo, KnrRCReadExcelFiles.StudentInfo studentInfo, String subjectToFind)
		...
		if (!processedRegNos.contains(regNo)) {
			...
			testCaseName = extentReport
					.createTest("Report Card Enrollment Page Actions for the following register number: " + regNo);
			...
			ExtentTest testCaseScenario = testCaseName
					.createNode("Starting testCase execution for the register number: " + regNo);
...
//        knrEnrollment.monitorNetworkDuringTest(testCaseName);
			...
			System.out.println(
					"Starting testCase6 execution for the reg: " + regNo + "and  for the Subject: " + subjectToFind);

			ExtentTest testCaseScenario = testCaseName
					.createNode("Starting testCase Validation for the register number: " + regNo
							+ " and for the Subject: " + subjectToFind);

			System.out.println("=========================");

			knrEnrollment.downloadPdfReportValidation(testCaseScenario, regNo, semester, regulation, paper1, paper2,
					paper3, paper4, theoryExamTotal, practicalExamTotal, grandTotal, subjectToFind);
		}
...
//for bnys pattern
	...
	public void knr_RC(String regNo, KnrRCReadExcelFiles.StudentInfo studentInfo, String subjectToFind)
		...
		for (Map.Entry<String, String> entry : subjects.entrySet()) {
			...
			System.out.println("This is a Normal Test Case6");
			System.out.println(
					"Starting testCase6 execution for the reg: " + regNo + " for the Subject: " + subjectToFind);
			System.out.println("=========================");

			KnrEnrollmentPageForAdditionalParameter.downloadPdfReportValidation(testCaseName, regNo, paper1, paper2,
					paper3, practicalExam, theoryExam, examTotal, subjectToFind, theoryInt, theoryTh, praticalInt,
					praticalPractical, praticalViva);
...
//Need to Create the separte methos in one class for the below ones exam,awardName,etcc
			...
			// Enrollment.processOtherDetails(examdate, awardName, semester, regulation,
			// examType);

			// Validate paper results and scores
			// Enrollment.validatePaperResults(Regno, paper1, paper2, paper3, praticalExam,
			// theroryExam, examTotal);
...
//for bnys pattern
	...
	public void knr_RC1() throws IOException {
		PatientPDFExtract PatientPDFExtract1 = new PatientPDFExtract();

		PatientPDFExtract1.method1(extentReport);
